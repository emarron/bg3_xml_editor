# basically a small variant of the rename_heads.py specifically for bodies.
# yes I know I should merge these.
...
def create_variants(given_name):
    variant_dict = {'name': given_name, 'skeleton_name': given_name + "_BASE",
                    ...
                    'texture_msk_name': given_name + "_MSK", 'texture_nm_name': given_name + "_NM"}
...
def create_variant_paths(path, dictionary):
    if path.exists():
        paths_dict = {'texture_clea_path': (path / dictionary['texture_clea_name']).with_suffix(".DDS"),
                      ...
                      'texture_msk_path': (path / dictionary['texture_msk_name']).with_suffix(".DDS"),
                      'texture_nm_path': (path / dictionary['texture_nm_name']).with_suffix(".DDS"),
                      'body_path': (path / dictionary['name']).with_suffix(".GR2")}
        return paths_dict
    else:
